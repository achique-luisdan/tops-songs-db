-- CALL core.extraction_split_artist_names( '{}');
-- CALL core.extraction_record_companies( '{}');
-- CALL core.extraction_songs( '{}');

CREATE OR REPLACE PROCEDURE core.extraction_split_artist_names(
...
BEGIN
	...
	FOR i IN array_lower(artist_names, 1)..array_upper(artist_names, 1) LOOP
		...
		IF is_new IS NULL OR is_new THEN
			RAISE NOTICE 'Creating new artist with name: %',TRIM(artist_names[i]);
...
CREATE OR REPLACE PROCEDURE core.extraction_record_companies(
...
BEGIN
	...
	FOR i IN array_lower(record_company_names, 1)..array_upper(record_company_names, 1) LOOP
		...
		IF is_new IS NULL OR is_new THEN
			RAISE NOTICE 'Creating new record company with name: %', record_company_names[i];
...
CREATE OR REPLACE PROCEDURE core.extraction_songs(
...
new_song_id INTEGER;
BEGIN
	SELECT ARRAY (SELECT id FROM core.tmp_weekly_tops) INTO songs_ids;
